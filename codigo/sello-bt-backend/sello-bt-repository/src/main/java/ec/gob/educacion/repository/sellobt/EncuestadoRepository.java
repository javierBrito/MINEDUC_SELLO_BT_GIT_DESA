...
public interface EncuestadoRepository extends JpaRepository<PenEstudiante, Integer>{
	 ...
	 * @author Belen changoluisa
	 */
	@Query(value = "select new ec.gob.educacion.sellobt.dto.EncuestadoDTO (p.estCodigo, p.estNombre, g.codigo , g.descripcion ) from PenEstudiante p, InsGrado  g where p.graCodigo = g.codigo and p.estCedula =:estCedula ")
	 ...
	 * @author Belen changoluisa
	 */
	@Query(value = "select new ec.gob.educacion.sellobt.dto.EncuestadoDTO (p.estCodigo, p.estNombre, g.codigo , g.descripcion ) from PenEstudiante p, InsGrado  g where p.graCodigo = g.codigo and p.estCodigo =:estCodigo ")
